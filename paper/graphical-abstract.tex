...
% Title
%\node[font=\sffamily\bfseries\Large, text=darkgray] (title) at (0, 14) {Graphical Abstract};
%{DeepWalk for Student Sectioning};
...
% Matrix background (width = 5, same as other boxes)
\fill[lightorange!50, rounded corners=3pt] (-7, 9.0) rectangle (-2, 12.2);

% Students (plain text, aligned with matrix rows)
...
% Course labels (with dots for more courses)
...
% Nodes (students as circles) - positioned in a circle layout
...
% Arrow from Student Graph to DeepWalk (rightward)
\draw[-{Stealth[length=3mm]}, blue1, thick] (-2.0, 5.5) -- (0.0, 5.5);

% ============================================================
% STAGE 3: Random Walks + Word2Vec (Middle Right)
% ============================================================
\node[font=\sffamily\bfseries, text=darkgray] at (2.5, 8.1) {DeepWalk};
...
% Arrow from DeepWalk to 2D Embeddings (downward)
...
%\node[circle, fill=blue1, text=white, minimum size=0.4cm, font=\sffamily\tiny] at (2.2, 1.7) {S4};

% Cluster 2 (orange) - bottom right
...
%\node[circle, fill=orange1, text=white, minimum size=0.4cm, font=\sffamily\tiny] at (3.5, 0.5) {S2};
...
% Arrow from 2D Embeddings to Results (leftward)
\draw[-{Stealth[length=3mm]}, blue1, thick] (0.0, 1.2) -- (-2.0, 1.2);

% ============================================================
% STAGE 5: Improved Clustering Results (Bottom Left)
% ============================================================
\node[font=\sffamily\bfseries, text=darkgray] at (-4.5, 3.1) {Results};

% Results background
\fill[gray!10, rounded corners=3pt] (-7, -0.3) rectangle (-2, 2.9);

% Result metrics
\node[font=\sffamily\small, text=darkgray, align=left] at (-4.5, 2.2) {\textbf{Key Result:}};

\node[draw=green1, thick, rounded corners=2pt, fill=white, minimum width=4cm, minimum height=0.7cm, font=\sffamily\small] at (-4.5, 1.5) {\textbf{Silhouette: +278\%}};
