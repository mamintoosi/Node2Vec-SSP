...
# Seed-42 embeddings of the PRIMARY Pareto configuration (p=1.0, q=0.5, d=2),
# saved by the run_pareto_validation.sh experiment.
OUT = os.path.join(ROOT, "results", "final_pareto_validation")
...
def main():
    ...
    print("-" * 75)
    for i in COURSES:
        emb = np.load(os.path.join(OUT, f"embeddings_course{i}_p1.0_q0.5_d2.npy"))
        m = cd[i]

        # Node2Vec embeddings (seed-42, primary p=1, q=0.5, d=2): 20 KMeans seeds
        ...
        AgglomerativeClustering(n_clusters=2).fit_predict(emb)
    ...
    out_json = os.path.join(OUT, "ari_stability_all_clusterers.json")
